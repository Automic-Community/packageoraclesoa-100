...
def printInputs(inputUser, inputProtocol, inputHost, inputPort, inputAppName, inputRevision, inputLabel, inputFailFlag):
...
inputAppName = sys.argv[6]
inputRevision = sys.argv[7]
inputFailFlag = sys.argv[8].lower()
inputLabel = rf_remove_pref(sys.argv[9], 6)
inputURL = inputProtocol + '://' + inputHost + ':' + inputPort

#print out list parameter
printInputs(inputUser, inputProtocol, inputHost, inputPort, inputAppName, inputRevision, inputLabel, inputFailFlag)
...
if compositeState == 'off':
    if inputFailFlag == 'no':
        print 'Application' + inputAppName + ' is already stopped, exit'
        disconnect()
        sys.exit()
    else:
        raise 'Application is already stopped, job failed'
else:
    ...
    try:
        if inputLabel == '':
            ...
            sca_stopComposite(inputHost, inputPort,inputUser,inputPassword,inputAppName,inputRevision)
        else:
            print 'stopComposite with label'
            sca_stopComposite(inputHost, inputPort,inputUser,inputPassword,inputAppName,inputRevision, inputLabel)
