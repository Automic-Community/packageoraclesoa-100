...
inputFailFlag = sys.argv[6].lower()
"&UC4RB_ORACLE_SOA_APP_NAME#" "&UC4RB_ORACLE_SOA_REVISION#" "&UC4RB_ORACLE_SOA_FAIL_RUNNING#" "&UC4RB_ORACLE_SOA_LABEL#" "&UC4RB_ORACLE_SOA_PARTITION#
def printInputs():
    print 'User: ' + sys.argv[1]
    print 'Host and port: ' + sys.argv[3]
    print 'Composite name: ' + inputAppName
    print 'Revision: ' + inputAppName
    print 'Label: ' + inputAppName
    print 'Partition: ' + inputAppName
...
depl_apps = ls('/AppDeployments/')
app_arr = depl_apps.replace('dr--   ', '').split('\n')  
appName = rf_prep_str(inputAppName)    

'''
if not checkAppExists(appName,'',''):  
     raise 'Application %s does not exist, Job failed' % inputAppName 
  
found_active_app = ''
cd('domainRuntime:/AppRuntimeStateRuntime/AppRuntimeStateRuntime')    
intendedState = cmo.getIntendedState(appName)    
if intendedState == 'STATE_ACTIVE':    
   found_active_app = appName     
   print 'Found active app:' + found_active_app      
if found_active_app == '':      
   i = 0  
   for app in app_arr:  
       app_name = app_arr[i].strip()  
       print 'app name:' + app_name   
       if (len(app_name) > len(appName)) and   (app_name[0:len(appName) + 1] == appName + '#'): 
            intendedState = cmo.getIntendedState(app_name)   
            print 'intendedState for ' + app_name + ' is '    
            print  intendedState    
            if intendedState == 'STATE_ACTIVE':    
                found_active_app = app_name       
                print 'Found active app:' + found_active_app     
                break                            
       i = i + 1  
...
try:
   ...
   if found_active_app != '':
       if inputFailFlag == "no":           
           print 'Application ' + found_active_app + ' is already active, exit'   
           disconnect('true')
           sys.exit()
       else:
...
'''
try:
   progress = sca_startComposite('vvnarasoa02','7003','weblogic','ventum2011','VacationRequest','1.0')
